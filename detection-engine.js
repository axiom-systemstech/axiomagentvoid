...
const DetectionEngine = (function() {
    ...
    // Patrones de jailbreak (intentos de romper restricciones)
    ...
    // Patrones de inyección de prompts (manipulación del sistema)
    ...
    // Patrones de ruido excesivo (texto basura)
    ...
    // Palabras comunes para detectar prompts vacíos o sin sentido
    ...
    function analyzePrompt(text) {
        ...
        // 5. Detectar texto sin significado (basura)
        ...
        // Calcular puntuación total (máx 100)
        let totalScore = jailbreakScore + injectionScore + noiseScore;
        ...
        if (totalScore >= 60) {
            level = "danger";
            recommendation = "NO ENVÍES ESTE PROMPT. Contiene patrones peligrosos de jailbreak o inyección de prompts. Revisa el contenido antes de enviarlo a cualquier API de IA.";
        } else if (totalScore >= 30) {
            level = "warning";
            recommendation = "RIESGO MODERADO. El prompt podría contener intentos de manipulación. Revísalo cuidadosamente antes de enviarlo.";
    ...
    function formatResult(result) {
        ...
        switch (result.level) {
            ...
            case "warning":
                ...
                break;
            case "danger":
                title = "🔴 PELIGRO - NO ENVIAR";
                icon = "🚫";
                break;
            default:
                title = "📝 ESPERANDO TEXTO";
                icon = "⌛";
        }
        
        let detail = `📊 Puntuación de riesgo: ${result.score}%\n\n`;
        detail += `📝 Palabras: ${result.wordCount} | Tokens estimados: ${result.tokenCount}\n\n`;
        
        if (result.detections.length > 0) {
            detail += `🔍 DETECCIONES (${result.detections.length}):\n`;
            for (let d of result.detections) {
                let typeIcon = d.type === "jailbreak" ? "🔓" : (d.type === "injection" ? "💉" : "📢");
                detail += `   ${typeIcon} ${d.name} (${d.weight}pts)\n`;
            }
            detail += `\n`;
        }
        
        detail += `💡 RECOMENDACIÓN:\n${result.recommendation}`;
